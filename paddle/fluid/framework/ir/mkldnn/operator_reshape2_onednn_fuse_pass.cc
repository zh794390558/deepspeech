...
void FuseOperatorReshape2OneDNNPass::ApplyImpl(Graph *graph) const {
  ...
  std::vector<std::pair<std::string, int>> ops_and_outputs = {{"fc", 1}, {"transpose2", 2}};
...
void FuseOperatorReshape2OneDNNPass::FuseReshape2(Graph *graph,
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t &subgraph,
    ...
    if (has_shape_tensor && reshape2_op->Op()->Input("ShapeTensor").size() > 0) {
      VLOG(4) << "Cannot fuse " << op_type << " and reshape2 because reshape2 dims are specified by ShapeTensor!";
      return;
    }

    if (has_shape_tensor_list && reshape2_op->Op()->Input("ShapeTensorList").size() > 0) {
      VLOG(4) << "Cannot fuse " << op_type << " and reshape2 because reshape2 dims are specified by ShapeTensorList!";
      return;
    }
