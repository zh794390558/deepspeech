...
void FuseOperatorUnsqueeze2OneDNNPass::FuseUnsqueeze2(
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t &subgraph,
    ...
    if (has_axes_tensor && unsqueeze2_op->Op()->Input("AxesTensor").size() > 0) {
      VLOG(4) << "Cannot fuse " << op_type << " and unsqueeze2 because unsqueeze2 dims are specified by AxesTensor!";
      return;
    }

    if (has_axes_tensor_list && unsqueeze2_op->Op()->Input("AxesTensorList").size() > 0) {
      VLOG(4) << "Cannot fuse " << op_type << " and unsqueeze2 because unsqueeze2 dims are specified by AxesTensorList!";
      return;
    }
